...
{	
	...
	public:	
		uint32 GetHomeId(){ return m_valueId->HomeId; }
		uint8 GetNodeId(){ return m_valueId->NodeId; }
		...
		Code GetCode() {
			return static_cast<Code>(m_byte);
			//(Code)Enum::ToObject( Code::typeid, m_byte); 
		}
	...
	internal:
		...
		{
			m_type = static_cast<NotificationType>(notification->GetType());//(Type)Enum::ToObject( Type::typeid, notification->GetType() );
			...
			//ref new ZWValueID( notification->GetValueID() );
		}
